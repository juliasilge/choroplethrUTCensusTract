...
get_all_ut_tracts = function()
{
        ut.counties = acs.fetch(geography    = geo.make(state = 49, county = "*"), 
                                table.number = "B01003")
        
        geo.make(state  = "UT", 
                 county = as.numeric(geography(ut.counties)[[3]]), 
                 tract  = "*")
}
...
get_ut_tract_acs_data = function(tableId, endyear=2013, span=5, column_idx=-1)
{
        acs.data = acs.fetch(geography    = get_all_ut_tracts(), 
...
#' Create a choropleth map of Utah Census Tracts from ACS data
#' 
#' Creates a choropleth of Utah Census Tracts the US Census' American Community Survey (ACS) data.  
...
#' @examples
#' \dontrun{
#' # per capita income, salt lake (fips code 6075)
#' ut_tract_choropleth_acs("B19301", county_zoom=49035)
#' 
