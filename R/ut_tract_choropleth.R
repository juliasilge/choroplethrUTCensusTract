#' An R6 object for creating choropleths of Utah Census Tracts.
#' @export
#' @importFrom dplyr left_join
#' @importFrom R6 R6Class
#' @importFrom choroplethr Choropleth
UtTractChoropleth = R6::R6Class("UtTractChoropleth",
                            inherit = choroplethr::Choropleth,
                            public = list(
                                    
                                    # initialize with a map of Utah Census Tracts
                                    initialize = function(user.df)
                                    {
                                            data(ut.tract.map, package="choroplethrUTCensusTract", envir=environment())
                                            super$initialize(ut.tract.map, user.df)
                                            
                                            if (private$has_invalid_regions)
                                            {
                                                    warning("Please see ?ut.tract.regions for a list of mappable regions")
                                            }
                                    },
                                    ...
                                    set_zoom_tract = function(county_zoom, tract_zoom)
                                    {
                                            ...
                                            data(ut.tract.regions, package="choroplethrUTCensusTract", envir=environment())
                                            ...
                                            if (!is.null(tract_zoom)) {
                                                    ...
                                                    stopifnot(all(county_zoom %in% unique(ut.tract.regions$county.fips.numeric)))
                                                    tracts = ut.tract.regions[ut.tract.regions$county.fips.numeric %in% county_zoom, "region"]
...
#' the "region" column must exactly match how census tracts are labelled in the "region" column in ?ut.tract.regions
...
#' match the names of tracts as they appear in the "region" column of ?ut.tract.regions.
...
#'                     reference_map = TRUE)
#' }
ut_tract_choropleth = function(df, 
                               title         = "", 
                               legend        = "", 
                               num_colors    = 7, 
                               tract_zoom    = NULL, 
